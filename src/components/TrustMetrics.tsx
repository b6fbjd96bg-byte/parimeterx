...
const AnimatedCounter = ({ value, suffix, isVisible }: { value: number; suffix: string; isVisible: boolean }) => {
  ...
  return <span className="text-4xl md:text-5xl font-bold text-primary text-glow">{displayValue}{suffix}</span>;
};
  ...
  return (
    <section className="py-20 relative">
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div 
          ref={animation.ref}
          className={`text-center mb-12 transition-all duration-700 ${animation.isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}
        >
          <span className="text-primary text-sm font-semibold tracking-widest uppercase mb-4 block">Trusted by Industry Leaders</span>
          <h2 className="text-3xl md:text-4xl font-bold">Proven <span className="text-primary text-glow">Results</span></h2>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
          {metrics.map((metric, index) => (
            ...
            >
              ...
              {/* Background glow */}
              <div className="absolute top-0 left-1/2 -translate-x-1/2 w-32 h-32 rounded-full bg-primary/10 blur-[40px] opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
              
              <div className="relative z-10">
                <div className="w-14 h-14 rounded-xl bg-primary/10 flex items-center justify-center mx-auto mb-4 group-hover:bg-primary/20 group-hover:scale-110 transition-all duration-300 border border-transparent group-hover:border-primary/20">
